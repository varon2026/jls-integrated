...
{ key:'semesterRecords',    table:'semester_records',     toRow:r=>({id:r.id,student_id:r.studentId,branch_id:r.branchId,semester_id:r.semesterId,class_name:r.className,class_label:r.classLabel,teacher:r.teacher,note:r.note,target_type:r.targetType,status:r.status,origin:r.origin,enroll_date:r.enrollDate,withdraw_date:r.withdrawDate,transfer:!!r.transfer,transfer_in:!!r.transferIn,transfer_to:r.transferTo||null,kind:r.kind||'regular',withdraw_reason:r.withdrawReason||null,withdraw_memo:r.withdrawMemo||null,grade:r.grade||null}),
  ...
  { key:'teacherChanges',     table:'teacher_changes',      toRow:c=>({id:c.id,branch_id:c.branchId,semester_id:c.semesterId,class_name:c.className,from_teacher:c.fromTeacher,to_teacher:c.toTeacher,date:c.date}),
   ...
   { key:'mcExemptions', table:'mc_exemptions', toRow:e=>({id:e.id,student_id:e.studentId,branch_id:e.branchId,semester_id:e.semesterId,stage:e.stage}),
    ...
    /* 시상관리 — MIP·BEST BOOK·BEST SPEECH 등록. sql/award_entries.sql 을 아직 안 돌렸어도 앱이 죽지 않게 optional.
       DT·AT 1등은 이 표에 안 남는다 — exam_scores에서 그때그때 계산해서 보여주는 값이라서다. */
...
function monthlyClosing(recs, months, activeMonths, splits, moves, recsAt){
  ...
  /* recsAt(월) → 그 달에 이 그룹이 실제로 맡고 있던 학생만. 학기 도중 담임이 바뀐 반 때문에 필요하다.
     예전엔 그룹 하나가 학기 내내 같은 명단을 쓴다고 보고 계산했다. 그래서 7월에 반을 넘겨받은
     강사의 6월 월초에 아직 남의 반이던 학생이 미리 들어갔고, 넘겨준 강사에게도 같은 학생이
     6월에 잡혀서 강사별 월초 합계가 그 반 인원만큼 분원 합계보다 커졌다.
...
let newThis, tiThis=0, wdThis, trThis;
    if(sp){
      ...
      newThis = R.filter(r=> enrollMonth(r)===m && !r.transferIn).length;
      tiThis  = R.filter(r=> enrollMonth(r)===m && r.transferIn).length;
...
/* 운정1은 에이스(중등)반이 없어서, 초5 체스→에이스로 넘어가는 학생은 반이 아니라
   ...
   그래서 이 전출-전입 짝만 따로 찾아서 운정1 이관 실적에 얹어준다. */
  ...
  if(linkedTo && prevId){
    db.semesterRecords.filter(r=>r.branchId===branchId && r.semesterId===prevId && (r.kind||'regular')!=='exam'
      ...
      if(cur && !isChess(cur.className)){
        list.push({rec:cur, early:true, prevClassName:prev.className, prevClassLabel:prev.classLabel, movedBranch:true});
      }
    });
  }
  return list;
...
if(session.role==='teacher'){
  ...
  if(root==='admin'){
    ...
    else if(session.role==='admin') renderHqAward();
...
function classLabel(raw){
  ...
  const parts = banParts(s);   // 닫는 대괄호가 빠진 반이름도 같은 규칙으로 쪼갠다
...
const AWARD_COLOR = {
  ...
  mip:   {bg:'var(--pos-soft)',   fg:'var(--pos)'},
  best_book:   {bg:'var(--neg-soft)', fg:'var(--neg)'},
  best_speech: {bg:'#eaf2fc',        fg:'#3e7fc9'},
};
function awardStripe(cat){ return cat ? AWARD_COLOR[cat].fg : 'transparent'; }
function renderTeacherAward(){
  const branchId = session.branchId;
  const teacher = session.teacherName;
  const b = getBranch(branchId);
  const semId = state.semId;
  ...
  if(!AWARD_SCORE_CACHE[semId]){
    el('content').innerHTML = `<div class="page-head"><h2>시상관리</h2></div><div style="padding:40px;text-align:center;color:var(--ink-3)">불러오는 중…</div>`;
    awardLoadExamScores(semId).then(()=>{ if(state.semId===semId) render(); });
    return;
  }
  ...
  const tipRows = (list, extra) => list.length
    ...
    : `<div style="padding:6px 0;color:var(--ink-3);font-size:11.5px">아직 없음</div>`;
  ...
  let html = `
    ...
    <div class="card" style="padding:18px 20px;overflow:visible;margin-bottom:16px">
      ...
      <div style="display:flex;gap:20px;flex-wrap:wrap;font-size:12.5px;font-weight:700;color:var(--ink-2)">
        <span class="award-tipwrap" style="position:relative"><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${AWARD_COLOR.dtat.fg};margin-right:6px"></span>DT·AT 자동 ${myScorers.length}명
          <div class="award-tip">${tipRows(myScorers, x=>x.score+'점')}</div></span>
        <span class="award-tipwrap" style="position:relative"><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${AWARD_COLOR.mip.fg};margin-right:6px"></span>MIP ${myEntries.filter(e=>e.category==='mip').length}명
          <div class="award-tip">${tipRows(myEntries.filter(e=>e.category==='mip').map(e=>({studentName:e.studentName,className:e.className})))}</div></span>
        <span class="award-tipwrap" style="position:relative"><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${AWARD_COLOR.best_book.fg};margin-right:6px"></span>BEST BOOK ${myEntries.filter(e=>e.category==='best_book').length}명
          <div class="award-tip">${tipRows(myEntries.filter(e=>e.category==='best_book').map(e=>({studentName:e.studentName,className:e.className})))}</div></span>
        <span class="award-tipwrap" style="position:relative"><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${AWARD_COLOR.best_speech.fg};margin-right:6px"></span>BEST SPEECH ${myEntries.filter(e=>e.category==='best_speech').length}명
          <div class="award-tip">${tipRows(myEntries.filter(e=>e.category==='best_speech').map(e=>({studentName:e.studentName,className:e.className})))}</div></span>
      </div>
    </div>
    ${awardTableMissing()?`<div style="border:1px solid #f3c9c9;background:#fdecec;border-radius:14px;padding:12px 16px;margin-bottom:16px;font-size:12.5px;color:#b8474b">아직 준비가 안 끝났습니다 — Supabase에서 <b>sql/award_entries.sql</b>을 실행한 뒤 새로고침해 주세요. 그 전까지는 MIP·BEST BOOK·BEST SPEECH 등록이 저장되지 않습니다.</div>`:''}
    <div style="display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start">`;

  const byClass = {};
  trecs.forEach(r=>{ (byClass[r.className]=byClass[r.className]||(byClass[r.className]=[])).push(r); });
  /* 카드 높이는 학생이 가장 많은 반 기준으로 맞춘다 — 그래야 어느 반도 목록이 스크롤 없이
     다 보이면서, 학생 적은 반은 그 반 카드가 짧아지는 대신 다 같은 높이로 나란히 보인다. */
  const maxClassCount = Math.max(1, ...Object.values(byClass).map(recs=>recs.length));
  const cardHeight = 100 + maxClassCount*42;
  Object.keys(byClass).forEach(className=>{
    const recs = byClass[className];
    const label = recs[0].classLabel || className;
    html += `<div class="card" style="width:360px;flex:none;height:${cardHeight}px;padding:0;overflow:hidden;display:flex;flex-direction:column">
      ...
      <div style="margin-top:auto;padding:9px 16px;border-top:1px solid var(--line-2);text-align:center;color:var(--ink-3);font-size:12px;font-weight:700;flex-shrink:0">${recs.length}명</div>
    </div>`;
  });
...
/* 이름 눌렀을 때 뜨는 선택 팝업 — MIP·BEST BOOK·BEST SPEECH 중 등록/해제 */
function awardOpenPicker(code, encName, encClass){
  const name = decodeURIComponent(encName);
  const semId = state.semId, branchId = session.branchId;
  const cur = (db.awardEntries||[]).filter(e=>e.branchId===branchId && e.semesterId===semId && e.studentCode===code);
  const has = cat => cur.some(e=>e.category===cat);
  const btn = (cat, label)=>{
    const on = has(cat);
    return `<button class="btn${on?' primary':''}" style="width:100%;text-align:left" onclick="${cat==='mip'?`awardGoMip('${code}','${encName}','${encClass}')`:`awardToggle('${code}','${encName}','${encClass}','${cat}')`}">${label}${on?' ✓':''}</button>`;
  };
  openModal(`
    <div class="modal-head"><div><h3>${esc(name)}</h3>
      <div class="mh-sub">${esc(decodeURIComponent(encClass))} · 시상 후보로 등록/해제</div></div>
      <button class="modal-x" onclick="closeModal()">×</button></div>
    <div class="modal-body" style="display:flex;flex-direction:column;gap:9px">
      ${btn('mip','MIP로 등록')}
      ${btn('best_book','BEST BOOK 후보로 등록')}
      ${btn('best_speech','BEST SPEECH 후보로 등록')}
    </div>`);
}
async function awardToggle(code, encName, encClass, category){
  const name = decodeURIComponent(encName), className = decodeURIComponent(encClass);
  const semId = state.semId, branchId = session.branchId, teacher = session.teacherName;
  const cur = (db.awardEntries||[]).find(e=>e.branchId===branchId && e.semesterId===semId && e.studentCode===code && e.category===category);
  let ok;
  if(cur) ok = await awardRemoveEntry(branchId, semId, code, category);
  else ok = await awardSaveEntry({ id:uid('awd'), branchId, semesterId:semId, studentCode:code, studentName:name, className, teacher, category, reason:null, createdBy:(session&&session.username)||null });
  toast(ok?(cur?'해제했습니다':'등록했습니다'):'저장 실패 — 다시 눌러 주세요', ok?'ok':'err');
  awardOpenPicker(code, encName, encClass);
  render();
}
function awardGoMip(code, encName, encClass){
  const name = decodeURIComponent(encName), className = decodeURIComponent(encClass);
  const semId = state.semId, branchId = session.branchId;
  const cur = (db.awardEntries||[]).find(e=>e.branchId===branchId && e.semesterId===semId && e.studentCode===code && e.category==='mip');
  openModal(`
    <div class="modal-head"><div style="display:flex;align-items:center;gap:8px">
      <button onclick="awardOpenPicker('${code}','${encName}','${encClass}')" style="border:none;background:var(--line-2);color:var(--ink-2);width:26px;height:26px;border-radius:50%;font-weight:800;cursor:pointer">‹</button>
      <h3>MIP — ${esc(name)}</h3></div>
      <button class="modal-x" onclick="closeModal()">×</button></div>
    <div class="modal-body">
      <p style="font-size:12px;color:var(--ink-3);margin-bottom:8px">한 학기동안 가장 발전한 점을 적어주세요</p>
      <textarea id="awardMipInput" rows="4" placeholder="예: 처음엔 숙제를 잘 안 해왔는데 이번 학기부터 매번 다 해오고 발표도 적극적으로 참여했어요" style="width:100%;box-sizing:border-box;border:1px solid var(--line);border-radius:10px;padding:10px 12px;font-family:inherit;font-size:12.5px;resize:none">${esc(cur?cur.reason||'':'')}</textarea>
      <div style="display:flex;gap:8px;margin-top:12px">
        <button class="btn primary" style="flex:1" onclick="awardSaveMip('${code}','${encName}','${encClass}')">저장</button>
        ${cur?`<button class="btn" onclick="awardUnregisterMip('${code}','${encName}','${encClass}')">해제</button>`:''}
      </div>
    </div>`);
}
async function awardSaveMip(code, encName, encClass){
  const name = decodeURIComponent(encName), className = decodeURIComponent(encClass);
  const semId = state.semId, branchId = session.branchId, teacher = session.teacherName;
  const val = (el('awardMipInput')||{}).value || '';
  const cur = (db.awardEntries||[]).find(e=>e.branchId===branchId && e.semesterId===semId && e.studentCode===code && e.category==='mip');
  const ok = await awardSaveEntry({ id:(cur&&cur.id)||uid('awd'), branchId, semesterId:semId, studentCode:code, studentName:name, className, teacher, category:'mip', reason:val, createdBy:(session&&session.username)||(cur&&cur.createdBy)||null });
  toast(ok?'MIP 등록했습니다':'저장 실패 — 다시 눌러 주세요', ok?'ok':'err');
  closeModal();
  render();
}
async function awardUnregisterMip(code, encName, encClass){
  const semId = state.semId, branchId = session.branchId;
  const ok = await awardRemoveEntry(branchId, semId, code, 'mip');
  toast(ok?'해제했습니다':'저장 실패 — 다시 눌러 주세요', ok?'ok':'err');
  closeModal();
  render();
}
...
/* ============================================================================
   17-3-3. 시상관리 — 본사
   분원 화면처럼 DT·AT/MIP/BEST SPEECH를 자세히 보여주지 않고 분원별 인원 숫자만 보여준다.
   BEST BOOK은 각 분원 투표에서 1~3등(동점이면 전부)으로 뽑힌 후보만 본사에 올라오고,
   본사 계정이 그 후보들에 투표한다(표 수 제한 없음, 누가 투표했는지는 따로 안 보여줌).
   본사 투표는 award_votes에 category='best_book_hq'로 저장한다(분원 투표와 섞이지 않음). */
function awardHqSetBranch(v){ state.awardHqBranch = v; render(); }
function renderHqAward(){
  const semId = state.semId;
  crumbs([{label:'시상관리'}]);
  if(!AWARD_SCORE_CACHE[semId]){
    el('content').innerHTML = `<div class="page-head"><h2>시상관리</h2></div><div style="padding:40px;text-align:center;color:var(--ink-3)">불러오는 중…</div>`;
  ...
  const sel = state.awardHqBranch || 'all';
  const colorIdx = {}; branches.forEach((b,i)=>{ colorIdx[b.id] = i % 8; });
  const myUsername = session && session.username;

  const stats = branches.map(b=>{
    const entries = (db.awardEntries||[]).filter(e=>e.branchId===b.id && e.semesterId===semId);
    const spCands = entries.filter(e=>e.category==='best_speech');
    const spTally = awardVoteTally(b.id, semId, 'best_speech');
    return {
      b,
      dtat: computeAwardTopScorers(AWARD_SCORE_CACHE[semId], b.name).length,
      mip: entries.filter(e=>e.category==='mip').length,
      spTotal: spCands.length,
      spDone: spTally.allVoted,
      spWinners: spTally.allVoted ? spTally.leaders.filter(c=>spCands.some(e=>e.studentCode===c)).length : 0,
    };
  });

  /* 분원 BEST BOOK 투표에서 1~3등(동점 전부)만 본사로 올라온다. 등수 = 나보다 표 많은 사람 수 + 1.
     표를 한 표도 못 받은 후보는 올리지 않는다(투표 전 분원의 후보가 전부 1등으로 올라오면 안 됨). */
  const hqRows = (db.awardVotes||[]).filter(v=>v.semesterId===semId && v.category==='best_book_hq');
  const book = [];
  branches.forEach(b=>{
    const tally = awardVoteTally(b.id, semId, 'best_book');
    const voted = (db.awardEntries||[])
      .filter(e=>e.branchId===b.id && e.semesterId===semId && e.category==='best_book')
      .map(e=>({e, v:tally.byCandidate[e.studentCode]||0})).filter(x=>x.v>0);
    voted.forEach(x=>{
      if(1 + voted.filter(y=>y.v>x.v).length > 3) return;
      const rows = hqRows.filter(v=>v.branchId===b.id && v.candidateCode===x.e.studentCode);
      book.push({ b, e:x.e, hq:rows.length, mine:rows.some(v=>v.voterUsername===myUsername) });
    });
  });
  /* 본사 순위는 전 분원 후보를 한꺼번에 놓고 본사 득표순으로 매긴다(동점이면 공동) */
  book.forEach(x=>{
    x.hqRank = x.hq>0 ? 1 + book.filter(y=>y.hq>x.hq).length : 0;
    x.hqTie = book.filter(y=>y.hq===x.hq).length>1;
  });

  const shownStats = stats.filter(s=>sel==='all' || s.b.id===sel);
  const shownBook = book.filter(x=>sel==='all' || x.b.id===sel);
  const tot = { dtat:0, mip:0, sp:0, book:0, spWait:false };
  shownStats.forEach(s=>{
    tot.dtat += s.dtat; tot.mip += s.mip; tot.sp += s.spWinners;
    if(s.spTotal>0 && !s.spDone) tot.spWait = true;
    tot.book += book.filter(x=>x.b.id===s.b.id).length;
  });

  const dash = '<span style="color:var(--ink-3)">-</span>';
  const num = (n, unit) => n>0 ? `<b style="font-size:13.5px">${n}</b>${unit}` : dash;
  const summaryRows = shownStats.map(s=>{
    const bk = book.filter(x=>x.b.id===s.b.id).length;
    const sp = s.spTotal===0 ? dash
      : (s.spDone ? num(s.spWinners,'명') : `<span style="display:inline-block;font-size:10.5px;font-weight:800;padding:3px 9px;border-radius:20px;background:var(--line-2);color:var(--ink-3)">투표중</span>`);
    return `<tr>
      <td style="font-weight:800;white-space:nowrap">${esc(s.b.name)}</td>
      <td class="award-next-col">${num(s.dtat,'명')}</td>
      <td class="award-next-mip">${num(s.mip,'명')}</td>
      <td class="award-next-speech">${sp}</td>
      <td class="award-next-book">${num(bk,'명')}</td>
    </tr>`;
  }).join('');

  const bookRows = shownBook.map((x,i)=>{
    const cc = 'award-hq-c'+colorIdx[x.b.id];
    const first = i===0 || shownBook[i-1].b.id!==x.b.id;
    const rank = x.hqRank>0 && x.hqRank<=3
      ? `<span style="font-size:11px;font-weight:800;padding:3px 10px;border-radius:20px;background:var(--brand-soft);color:var(--brand);white-space:nowrap">${x.hqTie?'공동 ':''}본사 ${x.hqRank}위</span>`
      : '<span style="color:var(--ink-3)">-</span>';
    return `<tr${first&&i>0?' style="border-top:2px solid var(--line)"':''}>
      <td style="font-weight:800;white-space:nowrap">${esc(x.b.name)}</td>
      <td style="font-weight:700">${esc(x.e.studentName)}</td>
      <td>${esc(awardClassLabelFor(x.b.id, semId, x.e.className))}</td>
      <td>${esc(x.e.teacher||'')}</td>
      <td class="${cc}">${x.hq>0?`<b style="font-size:13.5px">${x.hq}</b>표`:'<span style="color:var(--ink-3)">0표</span>'}</td>
      <td class="${cc}">${rank}</td>
      <td class="${cc}"><button style="border:none;border-radius:9px;padding:6px 13px;font-weight:800;font-size:11px;cursor:pointer;font-family:inherit;background:${x.mine?'var(--brand)':'var(--line-2)'};color:${x.mine?'#fff':'var(--ink-2)'}" onclick="awardVoteClick('${x.b.id}','${semId}','best_book_hq','${esc(x.e.studentCode)}',${x.mine})">${x.mine?'투표함 ✓':'투표하기'}</button></td>
    </tr>`;
  }).join('');
  ...
  <div style="display:flex;align-items:flex-start;margin:-24px 0 -24px -24px">
    <aside class="sidebar" style="position:sticky;top:0">
      <div class="sb-brand">
        <div class="sb-mark">JLS</div>
        <div><div class="t">시상관리</div><div class="s">본사</div></div>
      </div>
      <nav class="sb-nav">
        <div class="sb-item active">시상관리</div>
      </nav>
    ...
    <div style="flex:1;min-width:0;padding:24px 24px 24px 20px">
    <div class="page-head"><h2>시상관리</h2>
      <div class="sub">${esc(db.semesters.find(s=>s.id===semId)?.name||'')} · 분원별 수상 인원만 간단히 보고, BEST BOOK은 분원 1~3등 후보에 본사가 투표해요</div></div>
    ${awardTableMissing()?`<div style="border:1px solid #f3c9c9;background:#fdecec;border-radius:14px;padding:12px 16px;margin-bottom:16px;font-size:12.5px;color:#b8474b">아직 준비가 안 끝났습니다 — Supabase에서 <b>sql/award_entries.sql</b> · <b>sql/award_votes.sql</b>을 실행해 주세요.</div>`:''}

    <div style="margin-bottom:16px;display:flex;align-items:center;gap:9px;flex-wrap:wrap">
      <span style="font-size:12px;font-weight:800;color:var(--ink-3)">분원 선택</span>
      <select onchange="awardHqSetBranch(this.value)" style="border:1px solid var(--line);background:#fff;color:var(--ink);font-size:12.5px;font-weight:700;padding:7px 12px;border-radius:11px;font-family:inherit;cursor:pointer">
        <option value="all"${sel==='all'?' selected':''}>전체 분원</option>
        ${branches.map(b=>`<option value="${esc(b.id)}"${sel===b.id?' selected':''}>${esc(b.name)}</option>`).join('')}
      </select>
    </div>

    <div class="sect-head"><h3>분원별 수상 현황</h3><span class="cnt">${shownStats.length}개 분원</span></div>
    <div class="table-wrap" style="margin-bottom:20px"><table class="grid award-tbl">
      <thead><tr>
        <th>분원</th><th class="award-next-col">DT·AT</th><th class="award-next-mip">MIP</th><th class="award-next-speech">BEST SPEECH</th><th class="award-next-book">BEST BOOK (본사 제출)</th>
      </tr></thead>
      <tbody>${summaryRows || `<tr><td colspan="5" style="padding:16px;color:var(--ink-3)">분원이 없어요</td></tr>`}</tbody>
      <tfoot><tr style="background:var(--surface-2);font-weight:800">
        <td>${sel==='all'?'전체 합계':'합계'}</td><td>${tot.dtat}명</td><td>${tot.mip}명</td><td>${tot.sp}명${tot.spWait?' <span style="font-weight:600;color:var(--ink-3)">(확정분)</span>':''}</td><td>${tot.book}명</td>
      </tr></tfoot>
    </table></div>

    <div class="sect-head"><h3>BEST BOOK 본사 투표</h3><span class="cnt">${shownBook.length}명</span></div>
    <p style="margin:0 0 10px;font-size:11.5px;color:var(--ink-3)">각 분원 투표 1~3등(동점 전부)만 올라와요 · 본사 계정은 투표 수 제한이 없어요 · 본사 순위는 본사 득표순이에요</p>
    <div class="table-wrap" style="margin-bottom:20px"><table class="grid award-tbl">
      <thead><tr>
        <th>분원</th><th>학생명</th><th>이번학기 반</th><th>이번학기 담임</th><th>본사 득표</th><th>본사 순위</th><th>투표</th>
      </tr></thead>
      <tbody>${bookRows || `<tr><td colspan="7" style="padding:16px;color:var(--ink-3)">본사에 올라온 후보가 아직 없어요</td></tr>`}</tbody>
    </table></div>
    </div>
  </div>`;
}
...
function passTeacherRow(teacher, sortAgg, byGubun, branchId){
  ...
  const shortName = c=> banLevel(c) || c;   // 닫는 대괄호가 빠져도 레벨만 뽑는다
...
/* ------------------------------------------------------------------------
   시상관리 — 담임이 반배정표에서 이름 눌러 MIP·BEST BOOK·BEST SPEECH 등록/해제
   담임 계정은 읽기 전용(saveDB 막힘)이라 이 표도 retest_actions처럼 saveDB를
...
async function awardSaveEntry(row){
  ...
  try{
    const { error } = await sb.from('award_entries').upsert(T.toRow(row), { onConflict:'branch_id,semester_id,student_code,category' });
    if(error) throw error;
    const arr = db.awardEntries || (db.awardEntries=[]);
    const i = arr.findIndex(e=>e.branchId===row.branchId && e.semesterId===row.semesterId && e.studentCode===row.studentCode && e.category===row.category);
    if(i>=0) arr[i]=row; else arr.push(row);
    if(dbSnapshot){
      const s = dbSnapshot.awardEntries || (dbSnapshot.awardEntries=[]);
      const si = s.findIndex(e=>e.branchId===row.branchId && e.semesterId===row.semesterId && e.studentCode===row.studentCode && e.category===row.category);
      const copy = JSON.parse(JSON.stringify(row));
      if(si>=0) s[si]=copy; else s.push(copy);
    }
    return true;
...
async function awardRemoveEntry(branchId, semId, studentCode, category){
  ...
  if(!sb){ try{ initSupabase(); }catch(e){ console.error(e); return false; } }
  try{
    const { error } = await sb.from('award_entries').delete()
      .eq('branch_id', branchId).eq('semester_id', semId).eq('student_code', studentCode).eq('category', category);
    if(error) throw error;
    db.awardEntries = (db.awardEntries||[]).filter(e=>!(e.branchId===branchId && e.semesterId===semId && e.studentCode===studentCode && e.category===category));
    if(dbSnapshot){
      dbSnapshot.awardEntries = (dbSnapshot.awardEntries||[]).filter(e=>!(e.branchId===branchId && e.semesterId===semId && e.studentCode===studentCode && e.category===category));
    }
    return true;
...
/* 학교·학년 — 학년은 그 학기 명단에 적힌 값을 우선한다(학생 표의 학년은 최신 학기 기준이라 예전 학기와 다를 수 있다) */
function awardSchoolGrade(branchId, semId, code){
  const s = (db.students||[]).find(x=>x.code===code);
  if(!s) return ['',''];
  const rec = db.semesterRecords.find(r=>r.studentId===s.id && r.branchId===branchId && r.semesterId===semId && (r.kind||'regular')!=='exam');
  return [s.school||'', (rec&&rec.grade)||s.grade||''];
}
...
function awardExportExcel(){
  ...
  const sg = code => awardSchoolGrade(branchId, semId, code);
  ...
  const scoreRows = type => [['번호','학생명','회원코드','학교','학년','영어이름','이번학기 반','이번학기 담임','다음학기 반','다음학기 담임','다음학기 강의실','점수']]
    .concat(dtat.filter(s=>s.testType===type).map((s,i)=>{ codes.add(s.studentCode); return [i+1, s.studentName, s.studentCode, ...sg(s.studentCode), awardEnglishName(s.studentCode), s.classLabel, s.teacher, ...nextCols(s.studentCode), s.score]; }));

  const mip = entries.filter(e=>e.category==='mip');
  const mipRows = [['번호','학생명','회원코드','학교','학년','영어이름','이번학기 반','이번학기 담임','다음학기 반','다음학기 담임','다음학기 강의실','사유']]
    .concat(mip.map((e,i)=>{ codes.add(e.studentCode); return [i+1, e.studentName, e.studentCode, ...sg(e.studentCode), awardEnglishName(e.studentCode), awardClassLabelFor(branchId, semId, e.className), e.teacher||'', ...nextCols(e.studentCode), e.reason||'']; }));

  const voteRows = (category, finalLabel) => {
    const tally = awardVoteTally(branchId, semId, category);
    const list = entries.filter(e=>e.category===category);
    return [['번호','학생명','회원코드','학교','학년','영어이름','이번학기 반','이번학기 담임','다음학기 반','다음학기 담임','다음학기 강의실','득표','상태']]
      .concat(list.map((e,i)=>{
        ...
        return [i+1, e.studentName, e.studentCode, ...sg(e.studentCode), awardEnglishName(e.studentCode), awardClassLabelFor(branchId, semId, e.className), e.teacher||'', ...nextCols(e.studentCode), n, st];
      }));
  };
  ...
  add('DT', scoreRows('DT'), [6,10,14,20,8,14,22,14,22,14,14,8]);
  add('AT', scoreRows('AT'), [6,10,14,20,8,14,22,14,22,14,14,8]);
  add('MIP', mipRows, [6,10,14,20,8,14,22,14,22,14,14,60]);
  add('BEST SPEECH', voteRows('best_speech','당선'), [6,10,14,20,8,14,22,14,22,14,14,6,12]);
  add('BEST BOOK', voteRows('best_book','본사 제출 예정'), [6,10,14,20,8,14,22,14,22,14,14,6,14]);
...
function awardMaxVotes(category){
  if(category==='best_book_hq') return Infinity;   // 본사 계정은 표 수 제한 없음
  return category==='best_book' ? 3 : 2;
}
...
/* ------------------------------------------------------------------------
   ...
   exam_scores.branch 는 분원 아이디가 아니라 분원 이름 문자열로 저장돼 있다
   (grader.html이 올릴 때부터 그렇게 저장했다 — 여기서도 이름으로 맞춘다). */
let AWARD_SCORE_CACHE = {};   // semId → exam_scores 행 배열(DT·AT만)
async function awardLoadExamScores(semId){
  const semName = (db.semesters.find(s=>s.id===semId)||{}).name || '';
  if(!sb){ try{ initSupabase(); }catch(e){ console.error(e); AWARD_SCORE_CACHE[semId]=[]; return []; } }
  try{
    /* Supabase는 한 번에 1000행까지만 줘서, 전 분원 DT·AT 점수를 한 번에 받으면 뒤쪽이 조용히
       잘려 나간다(본사 화면은 전 분원을 다 보기 때문에 특히 위험). 끝까지 페이지를 넘겨 받는다. */
    const PAGE = 1000; let from = 0, all = [];
    while(true){
      const { data, error } = await sb.from('exam_scores').select('*')
        .eq('semester', semName).in('test_type', ['DT','AT']).range(from, from+PAGE-1);
      if(error) throw error;
