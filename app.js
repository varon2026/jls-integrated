...
function renderWonmuBody(){
  ...
  else if(v==='exam'){ if(cr) cr.innerHTML=`${home} › <b>시험채점</b>`; const _gq='sem='+encodeURIComponent(state.semId||'')+'&role='+encodeURIComponent(session.role||'')+'&branch='+encodeURIComponent(session.branchId||''); body.innerHTML='<div class="lt-back" onclick="wonmuGo(\'hub\')">‹ 원무 홈</div><iframe src="grader.html?'+_gq+'" title="시험채점" style="width:100%;height:calc(100vh - 175px);min-height:560px;border:1px solid var(--line);border-radius:14px;background:#fff"></iframe>'; }
